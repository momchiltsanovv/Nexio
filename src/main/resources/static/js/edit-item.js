...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const originalImageURL = preview.src;
    // Check if there's an existing image on page load (not a data URL and preview is visible)
    let hasOriginalImage = preview && 
                           preview.src && 
                           preview.src.trim() !== '' && 
                           !preview.src.includes('data:image') && 
    ...
    fileInput.addEventListener('change', function (e) {
        ...
        if (file) {
            // Track that a new file is selected
    ...
    if (form && descriptionField) {
        form.addEventListener('submit', function(e) {
            ...
            // Validate image - must have either existing image or new file
            const hasNewFile = fileInput && fileInput.files && fileInput.files.length > 0;
            // Check if there's an existing image (not a data URL, and preview is visible)
            const hasExistingImage = preview.src && 
                                    preview.src.trim() !== '' && 
                                    !preview.src.includes('data:image') && 
                                    preview.style.display !== 'none' &&
                                    preview.style.display !== '';
            ...
            if (!hasNewFile && !hasExistingImage) {
                ...
                // Show error message
                alert('Please upload a photo for your item or ensure an existing photo is displayed.');
